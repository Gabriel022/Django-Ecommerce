...
from lojafacco.models import Product, Commodity, SmartPhoneVariant, SmartPhoneModel, OperatingSystem, Camisetas
from lojafacco.models import Manufacturer, SmartCard
from lojafacco.models import CommodityInventory, SmartCardInventory, SmartPhoneInventory, CamisetasInventory


admin.site.site_header = "lojaFacco Administration"
...
#added code
class CamisetasInventoryAdmin(admin.StackedInline):
    model = CamisetasInventory
    extra = 0
#added code
...
class CamisetasAdmin(InvalidateProductCacheMixin, SearchProductIndexMixin, SortableAdminMixin, TranslatableAdmin, FrontendEditableAdminMixin,
                     PlaceholderAdminMixin, CMSPageAsCategoryMixin, PolymorphicChildModelAdmin):
    """
    Since our Commodity model inherits from polymorphic Product, we have to redefine its admin class.
    """
    ...
    filter_horizontal = ['cms_pages']
    inlines = [ProductImageInline, CamisetasInventoryAdmin]
    prepopulated_fields = {'slug': ['product_name']}
#added code
...
class SmartPhoneInline(admin.TabularInline):
    ...
    def variant_admin(self, obj):
        link = reverse('admin:lojafacco_smartphonevariant_change',
...
class SmartPhoneAdmin(InvalidateProductCacheMixin, SearchProductIndexMixin, SortableAdminMixin, TranslatableAdmin, FrontendEditableAdminMixin,
    ...
    def render_text_index(self, instance):
        template = get_template('search/indexes/lojafacco/commodity_text.txt')
...
class ProductAdmin(PolymorphicSortableAdminMixin, PolymorphicParentModelAdmin):
    base_model = Product
    child_models = [SmartPhoneModel, SmartCard, Commodity, Camisetas]
